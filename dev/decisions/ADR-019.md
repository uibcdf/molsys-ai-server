...
- include directories: `docs/`, `doc/`, `devguide/`,
- include root files: `README*.md`, `README*.rst`, `dev_guide.md`, `CHANGELOG*.md`, etc.,
- include extensions: `.md`, `.rst`, `.txt`, `.ipynb`.

By default we do **not** snapshot upstream `examples/` directories because they can lag behind the
current public documentation and may reinforce legacy APIs/aliases (e.g. `import molsysmt as msmt`,
or references to historical components like `MolSysSAK`). If needed for experimentation, include
...
Corpus selection (which directories/files are included) can be managed via an optional config file:

- `python dev/sync_rag_corpus.py --corpus-config <path>.toml ...`
