...
For the MolSys* ecosystem tools (MolSysMT, MolSysViewer, TopoMT, elastnet, pharmacophoremt, etc.),  
...
- a `requirements-dev.txt` file for developers who prefer `venv` + `pip` or `uv`.
...
- Makes it easier to maintain code quality.
 - conda/mamba users can get a fully reproducible environment (including MolSys* tools) with a single command.
