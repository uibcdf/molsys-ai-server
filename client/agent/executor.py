...
def create_default_executor() -> ToolExecutor:
    ...
    from .tools.core_tools import shell, sys_info

    executor.register(
        name="local.sys_info",
        func=sys_info,
        description="Return basic local system information (platform, python, cwd).",
    )
    executor.register(
        name="local.shell",
        func=shell,
        description="Run a shell command locally (requires explicit user approval).",
    )

    try:
        from .tools.molsysmt_tools import get_info
    except Exception:
        # Keep the agent usable in lightweight environments where molsysmt is not installed.
        return executor
